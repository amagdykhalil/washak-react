...
export default function Navbar({menu , loading , menuSetting , loadingSetting }) {
    ...
    return (
        <div className='sticky top-0 z-50'>
            ...
            {header_enable_switch != null && ( <header className={`text-white w-full flex items-center justify-center gap-4 text-base max-md:text-xs duration-500 transition-all ${scrolled ? 'max-h-0 overflow-hidden' : 'max-h-[55px]'}`} style={{ background: 'var(--main)' }}>
            ...
            <div className='bg-white' style={{ boxShadow: '0px 4px 4px 0px #CFCFCF40' }}>
                <nav className='container !h-[80px] mx-auto flex items-center justify-between py-4 !px-4 lg:px-0'>
                    ...
                    <div className='flex items-center gap-[15px]'>
                        ...
                        {cart_enable == 1 && (<Link id='cart-icon' to='/cart' className='relative cursor-pointer  p-1 w-8 h-8 duration-500'>
                            <span id='cart-count' className='absolute top-[-5px] right-[-5px] w-4 h-4 rounded-full text-white text-[10px] flex items-center justify-center' style={{ background: 'var(--main)' }}> 2 </span>
