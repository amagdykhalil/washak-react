        ...
        <div data-aos="zoom-in" className={` ${cn} max-sm:hidden  text-[#666666] text-lg max-md:text-base flex mt-[30px] !mb-[30px] items-center gap-2`}>
            ...
            {routes.map((route, idx) => (
                <div key={idx} className='flex items-center gap-2'>
                    {route.href ? (
                        <Link to={route.href} className='text-[var(--main)] cursor-pointer font-medium'>
                            {route.label}
                        </Link>
                    ) : (
                        <span className='text-gray-600'>{route.label}</span>
                    )}
                    {idx < routes.length - 1 && <span>/</span>}
                </div>
            ))}
        </div>
    );
